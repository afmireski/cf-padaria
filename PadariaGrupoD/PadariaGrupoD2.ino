...
int lastAumenta = 1; // ultima leitura com ruido
int botaoAumenta = 1; // estado do botao

int lastDiminui = 1; // ultima leitura com ruido
int botaoDiminui = 1; // estado do botao

int lastModo = 1; // ultima leitura com ruido
...
int main() {
  ...
  while(1) {
    ...
    if (leitura != lastAumenta) {
      aumentaAt = my_millis;
    }
  
    if ((my_millis - aumentaAt) > DEBOUNCE_INTERVAL) {
    ...
    leitura = (PINC & (1<<LEFT));
    if (leitura != lastDiminui) {
      diminuiAt = my_millis;
    }
  
    if ((my_millis - diminuiAt) > DEBOUNCE_INTERVAL) {
    ...
    if ((my_millis - modoAt) > DEBOUNCE_INTERVAL) {
