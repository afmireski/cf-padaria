...
#define MIN_ASSA_TEMP 60  // Temperatura mínimo de assamento
#define MAX_SOVA_TEMP 25  // Tempo máximo de sova
#define MAX_CRES_TEMP 34  // Tempo máximo de crescimento
#define MAX_ASSA_TEMP 150 // Tempo máximo de assamento
...
int actionState = 0;             // Define o que será configurado, ou tempo ou temperatura
...
int main()
{
  ...
  while (1)
  {
    ...
    if (flagState == 0)
    {
      ...
      TEMP2 = tempSovar % 10;  // Temperatura em unidade de graus;
      TEMP1 = tempSovar / 10;  // Temperatura em dezena de graus
      TEMP0 = tempSovar / 100; // Temperatura em centena de graus
    ...
    if (flagState == 1)
    {
      T3 = (tempoCrescer / 60000) % 10;        // Tempo em unidade de minuto;
      T2 = ((tempoCrescer / 60000) / 10) % 6;  // Tempo em dezena de minuto;
      T1 = ((tempoCrescer / 60000) / 60) % 10; // Tempo em unidade de hora;
      T0 = ((tempoCrescer / 60000) / 60) / 10; // Tempo em dezena de hora;

      TEMP2 = tempCrescer % 10;  // Temperatura em unidade de graus;
      TEMP1 = tempCrescer / 10;  // Temperatura em dezena de graus
      TEMP0 = tempCrescer / 100; // Temperatura em centena de graus
    ...
    if (flagState == 2)
    {
      ...
      TEMP2 = tempAssar % 10;  // Temperatura em unidade de graus;
      TEMP1 = tempAssar / 10;  // Temperatura em dezena de graus
      TEMP0 = tempAssar / 100; // Temperatura em centena de graus
    ...
    if (readAumenta != lastAumenta && (my_millis - aumentaAt) > DEBOUNCE_INTERVAL)
    {
      ...
      if (readAumenta == 0)
      {
        if (actionState == 0)
        {
          ...
          if ((flagState == 1) && (tempoCrescer < MAX_CRES_T))
          {
            tempoCrescer += 60000; // += 1 min
          }
        ...
        {
          if ((flagState == 0) && (tempSovar < MAX_SOVA_TEMP))
          {
            tempSovar += 1; // += 1 grau
          }
          if ((flagState == 1) && (tempCrescer < MAX_CRES_TEMP))
          {
            tempCrescer += 1; // += 1 grau
          }
          if ((flagState == 2) && (tempAssar < MAX_ASSA_TEMP))
          {
            tempAssar += 1; // += 1 grau
          }
        }
    ...
    if (readDiminui != lastDiminui && (my_millis - diminuiAt) > DEBOUNCE_INTERVAL)
    {
      ...
      if (readDiminui == 0)
      {
        if (actionState == 0)
        {
          ...
          if ((flagState == 1) && (tempoCrescer > 0))
          {
            tempoCrescer -= 60000; // -= 1min;
          }
        ...
        {
          if ((flagState == 0) && (tempSovar > MIN_SOVA_TEMP))
          {
            tempSovar -= 1; // -= 1 grau
          }
          if ((flagState == 1) && (tempCrescer > MIN_CRES_TEMP))
          {
            tempCrescer -= 1; // -= 1 grau
          }
          if ((flagState == 2) && (tempAssar > MIN_ASSA_TEMP))
          {
            tempAssar -= 1; // -= 1 grau
          }
        }
    ...
    if (readModo != lastModo && (my_millis - modoAt) > DEBOUNCE_INTERVAL)
    {
      ...
      if (readModo == 0)
      {
        ...
        if (flagState == 0)
        {
          if (actionState == 0)
          {
            actionState = 1; // Passa para config da temperatura
          }
          else
          {
            flagState = 1;   // Passa para modo de crescimento
            actionState = 0; // Passa para config do tempo
          }
          cmd_LCD(0x01, 0);
        }
        // Modo de crescimento
    ...
    if (flagState == 3)
    {
      if (execState == 0)
      {
        ...
        if ((my_millis - lastLCDRefresh) >= MILLIS_INTERVAL)
        {
          ...
          TEMP2 = tempDisplay % 10;  // Temperatura em unidade de graus;
          TEMP1 = tempDisplay / 10;  // Temperatura em dezena de graus
          TEMP0 = tempDisplay / 100; // Temperatura em centena de graus
      ...
      else if (execState == 1)
      {
        ...
        {
          startCres = true;
          tempoDisplay = tempoCrescer;
          tempDisplay = tempCrescer;
